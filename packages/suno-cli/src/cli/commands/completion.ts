...
const generateFish = (
	...
	const lines = [
		...
		...topLevelNames.map(
			(cmd) =>
				`complete -c ${cliName} -n '__fish_use_subcommand' -a '${cmd}' -d '${esc(commands[cmd].description)}'`,
		),
		"",
		"# Subcommands",
	];
	...
	for (const cmd of topLevelNames) {
		...
		if (subNames.length > 0) {
			...
			for (const [sub, desc] of Object.entries(subs)) {
				lines.push(
					`complete -c ${cliName} -n '${condition}' -a '${sub}' -d '${esc(desc)}'`,
				);
			}
			lines.push("");
		}
...
export const createCompletionCommand = (subCommands: SubCommandsDef) => {
	...
	return defineCommand({
		...
		run({ args }) {
			...
			switch (shell) {
				case "zsh":
					process.stdout.write(generateZsh(cliName, commands));
					break;
				case "bash":
					process.stdout.write(generateBash(cliName, commands));
					break;
				case "fish":
					process.stdout.write(generateFish(cliName, commands));
					break;
				default:
					process.stderr.write(
						`Unsupported shell: ${shell}. Use zsh, bash or fish.\n`,
					);
					process.exit(1);
			}
		},
